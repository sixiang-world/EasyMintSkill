...
**Load this reference when:** 需要对齐官方 skill 编写规范——命名约定、渐进式披露、工作流与反馈回路、评估驱动开发、脚本类 skill 的写法。

**中文用途：** 本文件是 [SKILL.md](../SKILL.md) 的**规范补充**。SKILL.md 讲 TDD 化的开发流程（先失败基线、反合理化），本文件讲**工程规范与信息架构**（怎么组织、怎么命名、怎么控制 token、怎么写脚本）。两者互补：一个管"对不对"，一个管"好不好用"。

**注意：** 本文已做平台中立化处理——去掉了对特定运行时/MCP 服务/厂商产品的指名依赖。凡涉及"运行时能力"之处，都改为按当前运行时实际约定判断。
...
**Testing considerations by model tier** (按能力档位，不指名具体厂商型号):

* **Fast / economical tier**: Does the Skill provide enough guidance?
* **Balanced tier**: Is the Skill clear and efficient?
* **Powerful-reasoning tier**: Does the Skill avoid over-explaining?

What works perfectly for the most capable model might need more detail for a smaller one. If you plan to use your Skill across multiple models, aim for instructions that work well with all of them.
...
* `description` - One-line description of when to use it (1024 characters maximum)

### Naming conventions

Use consistent naming patterns to make Skills easier to reference and discuss. **Gerund form** (verb + -ing) is recommended, as this clearly describes the activity or capability.
...
The `description` field enables Skill discovery. Following the TDD methodology in SKILL.md it should describe **when to use** it. Some ecosystems also include what the Skill does; **this package's convention is when-to-use only, starting with "Use when..."** — see SKILL.md's SDO section for why summarizing the workflow breaks discovery.
...
**Be specific and include key terms.** Include specific triggers/contexts for when to use it.

Each Skill has exactly one description field. The description is critical for skill selection: agents use it to choose the right Skill from potentially 100+ available Skills.
...
3. **Ask Agent A to create a Skill**: "Create a Skill that captures this BigQuery analysis pattern we just used. Include the table schemas, naming conventions, and the rule about filtering test accounts."
   ...
   Modern agents understand the Skill format and structure natively. Simply ask the agent to create a Skill and it will generate properly structured SKILL.md content with appropriate frontmatter and body content.
...
Runtime environments differ in what they permit. **Determine and state your target runtime's constraints explicitly** — for example, whether it can install packages from a registry and whether it has network access at all. Some environments have no network access and no runtime package installation.

List required packages in your SKILL.md and verify they're available in your runtime before relying on them.

### Runtime environment

Skills run in an environment with filesystem access, command execution, and code execution capabilities.
...
3. **Scripts executed efficiently**: Utility scripts can be executed without loading their full contents into context. Only the script's output consumes tokens
...
When the user asks about revenue, the agent reads SKILL.md, sees the reference to `reference/finance.md`, and reads just that file. The sales.md and product.md files remain on the filesystem, consuming zero context tokens until needed. This filesystem-based model is what enables progressive disclosure.

### Tool references

If your Skill uses external tools, always use **fully qualified tool names** as defined by your runtime, to avoid "tool not found" errors.
...
* [ ] Tested with every model tier you plan to use
