...
const PORT_FILE = process.env.KICKOFF_PORT_FILE || null;
...
function preferredPort() {
  if (process.env.KICKOFF_PORT) return Number(process.env.KICKOFF_PORT);
  if (PORT_FILE) {
    try {
...
const HOST = process.env.KICKOFF_HOST || '127.0.0.1';
const URL_HOST = process.env.KICKOFF_URL_HOST || (HOST === '127.0.0.1' ? 'localhost' : HOST);
const SESSION_DIR = process.env.KICKOFF_DIR || '/tmp/kickoff';
const CONTENT_DIR = path.join(SESSION_DIR, 'content');
const STATE_DIR = path.join(SESSION_DIR, 'state');
const KICKOFF_VERSION = readKickoffVersion();
let ownerPid = process.env.KICKOFF_OWNER_PID ? Number(process.env.KICKOFF_OWNER_PID) : null;
...
// Persisted alongside the port (KICKOFF_TOKEN_FILE) so a restart keeps the
// same key and an already-open tab's cookie still validates.
const TOKEN_FILE = process.env.KICKOFF_TOKEN_FILE || null;
...
function initialToken() {
  if (process.env.KICKOFF_TOKEN) {
    return { value: process.env.KICKOFF_TOKEN, source: 'env' };
  }
  if (TOKEN_FILE) {
    try {
...
let COOKIE_NAME = 'kickoff-key-' + PORT; // refined to the actual bound port in onListen
...
<html>
<head><meta charset="utf-8"><title>Kickoff Companion</title>
<style>
...
<body><!-- BRANDING --><h1>Kickoff Companion</h1>
...
function bootstrapPage(key) {
  ...
  return `<!DOCTYPE html>
<html>
<head><meta charset="utf-8"><title>Opening Kickoff Companion</title></head>
<body>
<script>
try { sessionStorage.setItem('kickoff-session-key', ${jsonKey}); } catch (e) {}
location.replace('/');
</script>
</body>
</html>`;
...
function brandMarkup() {
  // 本技能包不注入任何第三方品牌、logo 或外部请求。
  // 保留此函数是为了让 `<!-- BRANDING -->` 占位符能被清空，
  // 避免它作为注释残留在渲染结果里。
  return '';
}
...
// browser is already connected. Override the launcher with KICKOFF_OPEN_CMD.
let browserOpened = false;
function maybeOpenBrowser() {
  if (browserOpened) return;
  browserOpened = true;
  if (!process.env.KICKOFF_OPEN) return; // opt-in: only after the user approves the companion
  if (HOST !== '127.0.0.1' && HOST !== 'localhost') return;
  if (clients.size > 0) return; // the user already opened it
  const url = companionUrl(); // must carry the key or the gate 403s it
  const cp = require('child_process');
  // Operator-provided launcher: run as given (this env var is trusted operator input).
  if (process.env.KICKOFF_OPEN_CMD) {
    try { cp.exec(process.env.KICKOFF_OPEN_CMD + ' ' + JSON.stringify(url), () => {}); } catch (e) { /* best effort */ }
    return;
  }
...
// override with KICKOFF_IDLE_TIMEOUT_MS (start-server.sh: --idle-timeout-minutes).
const IDLE_TIMEOUT_MS = (() => {
  const ms = Number(process.env.KICKOFF_IDLE_TIMEOUT_MS);
...
const LIFECYCLE_CHECK_MS = (() => {
  const ms = Number(process.env.KICKOFF_LIFECYCLE_CHECK_MS);
...
function startServer() {
  ...
  server.on('error', (err) => {
    if (err.code === 'EADDRINUSE' && !triedFallback) {
      if (tokenSource === 'env') {
        console.error('Server failed to bind: preferred port is in use and KICKOFF_TOKEN is set; refusing fallback with explicit token');
        process.exit(1);
      }
      triedFallback = true;
      PORT = randomPort();
