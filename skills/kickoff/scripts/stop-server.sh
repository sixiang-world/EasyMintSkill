#!/usr/bin/env bash
# Stop the companion server and clean up
# Usage: stop-server.sh <session_dir>
#
# Kills the server process. Only deletes session directory if it's
# under /tmp (ephemeral). Persistent directories (.agentskill/) are
...
command_has_server_id() {
  local pid="$1"
  local expected="$2"
  local expected_arg="--kickoff-server-id=$expected"
  if [[ -r "/proc/$pid/cmdline" ]]; then
    local arg
