...
| 只能在用户**显式点名**时才加载 skill | ⚠️ **不能** —— `using-easymint` 的强制触发不成立 |
| 完全没有 skill 机制，只有系统提示词 | ⚠️ **不能** —— 需要你自己把入口技能贴进系统提示词 |

**第二、三种情况下，本包的「两层强制触发」设计会退化为「建议」。** 这时你有三个选择：

1. **接入本目录的注入器**（推荐，见下）
2. **手工引导**：在每个新会话开头对 Agent 说一句「先读 `using-easymint`，然后再回应我」
...
在会话启动时，把入口技能 `using-easymint` 的**全文**注入到 Agent 的上下文里 —— 让"先查 skill"这条规则在它看到你的消息之前就已经在它的上下文里。
...
宿主平台
  └→ [本目录的注入器]  ← 可选，解决"入口技能怎么被触发"
       └→ 把 using-easymint 注入上下文
            └→ using-easymint（调度层：判断用哪个 skill）
                 └→ kickoff（创作起手层）
                      └→ ... 后续技能
```
